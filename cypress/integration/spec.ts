  ...
  describe('Angular Library', () => {
    describe('Query support', () => {
      it('should display the query editor', () => {
        ...
        cy.contains('Observation?code=http://loinc.org|65972-2&date=gt{{today()-7 days}}&patient={{%patient.id}}');

        // Add a new code
        cy.get('#autocomplete-2').type('weight');
        cy.contains('29463-7').click();
        cy.contains('Weight - 29463-7');
        cy.contains('Observation?code=http://loinc.org|65972-2%2Chttp://loinc.org|29463-7&date=gt{{today()-7 days}}&patient={{%patient.id}}');

        // Remove first code
        cy.get(':nth-child(1) > button > span').click();
        cy.contains('Observation?code=http://loinc.org|29463-7&date=gt{{today()-7 days}}&patient={{%patient.id}}');
      });
    });
  });
});
